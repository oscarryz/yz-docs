...
- [ ] **[YZC-0048] Flaky test 51 — concurrent output ordering** — `51_lazy_scalar_variable` fails intermittently because `Counter.Increment(n)` and `P.Call()` are batched into the same `_bg1` BocGroup (introduced at d84e78b, concurrent boc call batching in YZC-0033); they run on different cowns so their print order is non-deterministic. Before d84e78b each was in its own sequential group. The expected output `"incrementing 1\nincrementing 1\nabout to print\n2"` assumes `p.call()` prints after both counter calls — an ordering the semantics do not guarantee. Fix: either update the runtime test harness to accept unordered concurrent output lines, or restructure the test so `p.call()` has a data dependency that forces sequencing.
