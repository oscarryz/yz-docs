...
- [x] **Phase E.1 — Implicit BocGroup per scope** — Done. Statement-position boc calls batched into implicit BocGroups; split-BocGroup pattern for cown release before Wait; ScheduleAsSuccessor for sub-boc calls on held cowns. 51 golden tests passing.

- [x] **Phase E.2 — Intrinsic lazy scalar types (thunkenization)** — Done. `std.Int/String/Bool/Decimal/Unit` carry an optional `lazy *lazyX` pointer; `LazyT()` constructors wrap `*Thunk[T]`; `GoWait(Waitable)` replaces Go+Force; `.Force()` gone from all generated code except entry point. 51 golden tests passing.

- [ ] **Phase E.3 — De-thunkenize scalars; uniform GoWait/GoStore for all types** — Agreed direction after E.2 review. The lazy fields in scalar types (`lazy *lazyInt` etc.) are implementation overhead that doesn't extend cleanly to user-defined types. Remove them; use a uniform BocGroup-based mechanism instead.
  - Remove `lazy *lazyX` fields from `std.Int/String/Bool/Decimal/Unit`; remove `LazyT()` constructors; remove `Await()` from scalars
  - Boc methods returning scalars go back to `*Thunk[T]` internally
  - Add `BocGroup.GoStore[T](th *Thunk[T], dest *T)` — goroutine forces thunk, assigns result; after `Wait()` dest is a plain value
  - `GoWait` stays for Unit-returning (fire-and-forget) calls
  - Generated pattern for `n Int = counter.value()`: `var n std.Int; _bg.GoStore(Counter.Value(), &n); _bg.Wait()`
  - Same pattern for user-defined types (eliminates `thunkVars` bookkeeping entirely)
  - Prerequisite for E.2.x "uppering": plain scalar types with no hidden runtime fields can be moved to Yz source with `compile-time:[Native]` annotation

  - [ ] **E.2.x — Scalar types defined in Yz source** — After E.3 removes hidden fields, `Int/String/Bool/Decimal/Unit` can be moved from Go source to Yz source in `stdlib/` with `compile-time:[Native]` annotation (or similar infostring). Native operations annotated per method (e.g. `` `go-operator: 'int64.+'` ``). Higher-level methods (`times`, `to`, `clamp`) become plain Yz. This removes all primitive-type special-casing from the compiler.
