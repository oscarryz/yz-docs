...
## Concurrency Model — Foundational Fixes (in order)

These two changes address fundamental correctness issues in the generated code and should be done before extending collections or other features.

- [ ] **Phase E.1 — Implicit BocGroup per scope** — Currently, statement-position boc calls in generated boc bodies are Forced inline to enforce sequential ordering. This is wrong: it makes concurrency sequential and causes deadlock in cown-recursive bocs (golden 39). The correct model is that every boc body has an implicit `_bg BocGroup`; statement-position calls (result not used) register their thunk with `_bg`; the scope exits with `_bg.Wait()`. Force is then only inserted at actual value-use points (the `thunkVars` mechanism already does this for declared variables). Changes: lowerer emits implicit BocGroup in `lowerBocBody`, `lowerClosureBody`, `lowerMainBoc`; remove statement-position `ForceExpr` wrapper from IR; regenerate all golden tests. This also fixes the cown-recursive deadlock in golden 39 for non-cown cases.

- [ ] **Phase E.2 — Intrinsic lazy value types (thunkenization)** — Currently `std.Int`, `std.String`, `std.Bool`, etc. are concrete Go values; laziness is a separate `*Thunk[T]` wrapper added by codegen. The real model: value types are themselves lazy. `std.Int` holds either a resolved value or a computation; operations like `Plus`, `Eqeq` return new lazy `std.Int` values that force their inputs when they are forced. Force cascades inward from IO boundaries (`std.Print` forces its argument, which forces its sub-expressions, etc.). The `*Thunk[T]` wrapper remains only for boc scheduling results (has a goroutine completed?). Changes: redesign `std.Int`, `std.String`, `std.Bool`, `std.Unit`, `std.Decimal` in `runtime/rt/`; remove explicit value `.Force()` calls from codegen; `thunkVars` mechanism simplified or removed; all golden tests regenerated. Prerequisite: Phase E.1 must be in place so structured concurrency is correct before changing how value forces propagate.

  - [ ] **E.2 first step — Split `yz/runtime/rt` into `rt` and `std`** — Currently the single `yz/runtime/rt` package (aliased `std` in generated code) mixes user-facing stdlib types with internal runtime machinery. Split into two packages: `yz/runtime/rt` — internal only: `Thunk[T]`, `Cown`, `Schedule`, `ScheduleMulti`, `ScheduleFlatten`, `BocGroup`; `yz/runtime/std` — user-facing stdlib: `Int`, `Bool`, `String`, `Decimal`, `Unit`, `Array[T]`, `Dict[K,V]`, `Print`, `http`. Generated Go code imports both (`std "yz/runtime/std"`, `rt "yz/runtime/rt"`); user types keep `std.` prefix, runtime internals move to `rt.` prefix. Do this as the first mechanical step of E.2 before redesigning the type representations, so the boundary is established before types move to Yz source.

  - [ ] **E.2 stdlib types as Yz source with intrinsic bodies** — Once the package split is in place, user-facing types (`Int`, `Bool`, `String`, etc.) move from Go source in `yz/runtime/std/` to Yz source in a `stdlib/` directory compiled as part of the runtime. Methods whose bodies are native operations are marked with an infostring annotation (e.g. `` `intrinsic: "add"` ``) and the compiler substitutes the Go implementation. Higher-level methods (`times`, `to`, `clamp`, etc.) are plain Yz built on those intrinsics. `Bool` becomes a sum type (`True()`, `False()`) with `?`, `&&`, `||`, `not` as Yz methods dispatching via `match`. Literal hooks (`42` → `Int`, `true` → `Bool.True()`, `[1,2,3]` → `Array`) remain compiler magic. This removes all special-casing of primitive types from the compiler.

- [ ] **Reentrant inline calls unsafe in closures passed to other bocs** — `emitStmt` inlines held-cown method calls based on codegen context (`heldCowns` set), not execution context. If a closure is generated inside a ScheduleMulti body and passed as an argument to another boc, the closure will contain direct sync-body calls (e.g. `src.balanceMinusEq(amount)`) that bypass cown acquisition. When the receiving boc calls the closure, it won't hold the required cown — data race. Fix: when emitting a closure body that is passed as a call argument (not the ScheduleMulti callback itself), create a sub-generator with `heldCowns = nil` so the async path is used instead. Dormant until HOF closures or spawn operate on cown-bearing types (e.g. `accounts.each({ a | a.balance-=(fee) })`).
...
- [x] **BocWithSig fresh instance per call site** — `transfer(alice, bob, 10)` lowers as `Transfer.Call(alice, bob, 10)`, reusing the global singleton `Transfer`. All concurrent calls serialize through `Transfer.Cown`, even when src/dst accounts are disjoint. Fix: in `lowerCall`, when the resolved symbol is a `BocType` singleton and it has `ExtraCowns` (i.e. multi-cown), emit `(&_transferBoc{}).Call(args)` instead of `Transfer.Call(args)` so each call gets its own uncontested `self.Cown`. The effective serialization comes only from `src.Cown` and `dst.Cown` acquired atomically via `ScheduleMulti`. Golden test 45.

- [x] **Struct-field cown acquisition in method bodies** — A method like `run: { src.balance = src.balance - amount }` inside a struct boc accesses fields whose types are other bocs (e.g. `src Account`, `dst Account`). The method currently acquires only `self.Cown`, leaving `src.Cown` and `dst.Cown` unprotected → data race. Fix: in `lowerMethod` (and any method lowering path), scan the receiver struct's fields for entries whose resolved `sema.Type` is `*sema.StructType` with `!IsInterface`. For each such field, add `"&self." + fieldName + ".Cown"` to the `ThunkExpr.ExtraCowns` slice. The codegen already handles `ExtraCowns` via `ScheduleMulti`. Together with the fresh-instance fix above, this makes `Transfer(alice,bob,10)` acquire `{self, alice, dst}` atomically — the correct BOC model. Golden test 44.
