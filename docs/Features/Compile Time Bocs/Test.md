...
`http_test.yz` is compiled as a fragment of `net.http` — it can see `http`'s internal (non-public) fields. The `Test` compile-time boc wires it into the test runner.

## Test isolation

Files with `!:[Test]` are excluded from production builds. The test runner source root (or a build condition) separates them.
