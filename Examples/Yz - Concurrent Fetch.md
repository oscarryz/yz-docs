...
- **Yz** takes ~2 seconds — all four fire immediately as concurrent goroutines.
...
main: {
    ...
    // All four fire immediately as concurrent goroutines.
...
spawns a goroutine and returns a **thunk** — a lazy handle to the future result.
...
**The type of `a` is still `String`**, not "thunk" or "future" or "promise".
...
A thunk is **forced** (its goroutine is joined and the value materialised) the moment
the value is actually needed — here, when passed to `print`:
...
| Language     | Concurrent fetch requires                              |
|--------------|--------------------------------------------------------|
| Go           | `goroutine`, channels or `sync.WaitGroup`              |
| JavaScript   | `Promise.all([fetch(...), fetch(...)])`                |
| Python       | `asyncio.gather(fetch(...), fetch(...))`               |
| Yz           | just assign: `a: http.get(url)` — thunks do the rest  |
