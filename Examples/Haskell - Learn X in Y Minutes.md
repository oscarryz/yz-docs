...
my_map: { array [A](); f #(A, B)
  array.length() == 0 ? { return [B]() }
  r: my_map(sub_array(array, 0), f)
  push(r, f(array[0]), 0)
}

(1.to(5)).collect({ x Int; x + 2 })
...
my_point: CartesianPoint(7.0, 10.0)
...
distance_from_origin: {
    p: { x Float; y Float }
    match {
        meta_data(p).block_name == 'CartesianPoint2D' => sqrt(p.x ** 2 + p.y ** 2)
    }, {
